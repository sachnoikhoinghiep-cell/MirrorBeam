...
/* reads runtime commands from stdin; used by the iPhone Mirror app to change
 * the videoflip direction without restarting (which would drop the AirPlay client):
 *   vd <identity|90r|90l|180|horiz|vert|ul-lr|ur-ll>
 */
static void *stdin_command_loop (void *arg) {
    char line[128];
    while (fgets(line, sizeof(line), stdin)) {
        if (strncmp(line, "vd ", 3) == 0) {
            char *dir = line + 3;
            char *end = dir + strlen(dir);
            while (end > dir && (end[-1] == '\n' || end[-1] == '\r' || end[-1] == ' ')) *--end = '\0';
            video_renderer_set_direction(dir);
            printf("videoflip direction set to %s\n", dir);
            fflush(stdout);
        }
    }
    return NULL;
...
int main (int argc, char *argv[]) {
    ...
    pthread_t stdin_cmd_thread;
    if (pthread_create(&stdin_cmd_thread, NULL, stdin_command_loop, NULL) == 0) {
        pthread_detach(stdin_cmd_thread);
    }
