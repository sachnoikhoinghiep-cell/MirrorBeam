...
static const char *videoflip_direction_nick (const videoflip_t *flip, const videoflip_t *rot) {
    switch (*flip) {
    case INVERT:
        switch (*rot)  {
        case LEFT:  return "90r";
        case RIGHT: return "90l";
        default:    return "180";
        }
    case HFLIP:
        switch (*rot) {
        case LEFT:  return "ul-lr";
        case RIGHT: return "ur-ll";
        default:    return "horiz";
        }
    case VFLIP:
        switch (*rot) {
        case LEFT:  return "ur-ll";
        case RIGHT: return "ul-lr";
        default:    return "vert";
        }
    default:
        switch (*rot) {
        case LEFT:  return "90l";
        case RIGHT: return "90r";
...
static void append_videoflip (GString *launch, const videoflip_t *flip, const videoflip_t *rot) {
    /* always insert a named videoflip so its direction can be changed at runtime
     * via video_renderer_set_direction() without rebuilding the pipeline */
    g_string_append(launch, "videoflip name=uxflip video-direction=");
    g_string_append(launch, videoflip_direction_nick(flip, rot));
    g_string_append(launch, " ! ");
}

void video_renderer_set_direction (const char *direction) {
    for (int i = 0; i < n_renderers; i++) {
        if (!renderer_type[i] || !renderer_type[i]->pipeline) continue;
        GstElement *flip = gst_bin_get_by_name(GST_BIN(renderer_type[i]->pipeline), "uxflip");
        if (flip) {
            gst_util_set_object_arg(G_OBJECT(flip), "video-direction", direction);
